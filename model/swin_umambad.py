...
class SwinUMambaD_text(nn.Module):
    def __init__(self, vss_args, decoder_args, num_heads_gat=1,
                 num_attn_heads=1,
                 clip_model='ViT-B/32',
                 device='cpu'):
        super().__init__()
        self.vssm_encoder = VSSMEncoder(**vss_args)
        self.decoder = UNetResDecoder(**decoder_args)
        self.visual_feature_dim = 768
        if clip_model == 'ViT-B/32':
            self.textual_feature_dim = 512
        if clip_model == "RN50":
            self.textual_feature_dim = 1024
        self.intermediate_dim = 256
        self.out_dim = 128
        self.attention = CrossAttentionWithGating(visual_dim=self.visual_feature_dim, text_dim=self.textual_feature_dim,
                                                  embed_dim=self.intermediate_dim, output_dim=self.out_dim,
                                                  clip_model=clip_model, num_heads_gat=num_heads_gat,
                                                  num_attn_heads=num_attn_heads, device=device)

    def forward(self, x, anatomical_labels, disease_labels):
        skips = self.vssm_encoder(x)
        attention_output = self.attention(skips[4], anatomical_labels, disease_labels)
        skips[4] = attention_output
        out = self.decoder(skips)
        return out
...
if __name__ == "__main__":
    ...
    print(model)
    summary(model, (4, 3, 224, 224), device='cuda')